...
public:
    ...
    Node(Node *parent, unsigned int action, float prob, unsigned int num_actions);

    unsigned int best_child(float c_puct, float c_virtual_loss);

    float PUCT(float c_puct, float c_virtual_loss, unsigned int total_child_visits) const;
...
class MCTS {
public:
    MCTS(NeuralNetwork *neural_network, unsigned int num_threads, float c_puct, float c_virtual_loss, unsigned int num_actions);
